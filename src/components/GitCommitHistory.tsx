...
const ContributionCell = memo(({
  ...
  return (
    <motion.div
      className="w-full aspect-square rounded-[1px] relative group"
      style={{
...
export default function GitCommitHistory() {
  ...
  const getContributionColor = useCallback((level: number, isHovered: boolean): string => {
    ...
    return (isHovered ? HOVER_COLORS[safeLevel] : BASE_COLORS[safeLevel]) ?? BASE_COLORS[0]!;
  }, []);
  ...
  useEffect(() => {
    ...
    const loadGitHubData = async () => {
      ...
      try {
        const { contributions: contributionData, stats } = await fetchGitHubContributions();

        if (!isMounted) return;

        // Transform flat array into weeks array
        const weeks: ContributionDay[][] = [];
        let currentWeek: ContributionDay[] = [];

        // Calculate how many days to skip at the beginning to align with Sunday
        const firstDate = new Date(contributionData[0]?.date || new Date());
        const firstDayOfWeek = firstDate.getDay();

        // Add empty days at the beginning if the first date isn't a Sunday
        ...
        if (isMounted) {
          ...
          setIsLoading(false);
          // Ensure we show the component even if there's an error (it will show empty state)
          setIsLoaded(true);
        }
      }
    };

    // Load data immediately without waiting for idle callback to ensure it loads on navigation
    loadGitHubData();
        ...
        <div className="relative z-10 w-full">
          <div className="w-full">
            <div className="flex gap-2">
              <div className="flex flex-col justify-between pt-6 text-[10px] text-zinc-500/70 pb-1">
                {weekdayLabels}
              </div>

              <div className="flex-1 relative">
                {/* Month labels */}
                <div className="relative text-[10px] text-zinc-500/70 h-5 w-full mb-1">
                  {monthLabels.map((label, i) => (
                    <div
                      key={`month-${i}`}
                      className="absolute transform -translate-x-1/2"
                      style={{
                        left: `${(label.position / contributions.length) * 100}%`,
                      }}
                    >
                      {label.month}
                    </div>
                ...
                <div className="flex w-full gap-[2px]">
                  {visibleContributions.map((week, weekIndex) => (
                    <div key={`week-${weekIndex}`} className="flex-1 min-w-0">
                      <ContributionWeek
                        week={week}
                        getContributionColor={getContributionColor}
                        getGlowIntensity={getGlowIntensity}
            ...
            {/* Legend - simplified */}
            <div className="flex items-center justify-end mt-2 text-[8px] text-zinc-500/70">
