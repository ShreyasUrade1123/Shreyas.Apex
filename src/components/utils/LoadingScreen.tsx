...
export default function LoadingScreen({ children }: { children: React.ReactNode }) {
  ...
  // Optimized loading progress - no artificial delays
  useEffect(() => {
    // Fast progress animation
    const interval = setInterval(() => {
      setProgress((prev) => {
        const increment = (100 - prev) * 0.2; // Faster increment
        const newProgress = prev + increment;
        return newProgress > 99 ? 99 : newProgress;
      });
    }, 30); // Faster interval

    // Complete loading immediately when progress bar visually fills
    const timeout = setTimeout(() => {
      clearInterval(interval);
      setProgress(100);
      setLoading(false);
      setShowStartButton(true); // Show button immediately
    }, 800); // Just enough time for smooth progress bar animation
  ...
  const handleStart = () => {
    ...
    // Play music when audio is ready
    if (audioRef.current) {
    ...
    }, 200); // Near-instant start
  };

  // Optimized particle count for better performance
  const particles = useMemo(() => Array.from({ length: 30 }, (_, i) => i), []); // Reduced from 100 to 30 for performance
