  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      if (map.current) {
        ...
        // Helper function to safely set paint property only if layer exists
        const safeSetPaintProperty = (
          ...
          try {
            ...
            if (layer) {
              // @ts-ignore - Mapbox layer properties are dynamic based on style
              mapInstance.setPaintProperty(layerId, property, value);
            }
          } catch (error) {
            // Silently ignore errors for non-existent layers
            console.debug(`Layer '${layerId}' not found in map style`);
          }
        };

        // Điều chỉnh màu sắc của nước
        safeSetPaintProperty('water', 'fill-color', '#1A2234');

        // Điều chỉnh màu sắc của đất liền
        safeSetPaintProperty('land', 'background-color', '#242F3E');

        // Điều chỉnh màu sắc của đường - try multiple possible layer names
        const roadLayers = [
          'road-primary',
        ...
        roadLayers.forEach(layer => {
          safeSetPaintProperty(layer, 'line-color', '#3A4556');
        });
        ...
        labelLayers.forEach(layer => {
          safeSetPaintProperty(layer, 'text-color', '#8D9CAD');
        });

        // Điều chỉnh màu sắc của công viên
        const parkLayers = ['park', 'landuse', 'national-park'];
        parkLayers.forEach(layer => {
          safeSetPaintProperty(layer, 'fill-color', '#263240');
        });

        // Điều chỉnh màu sắc của tòa nhà
        const buildingLayers = ['building', 'building-extrusion'];
        buildingLayers.forEach(layer => {
          safeSetPaintProperty(layer, 'fill-color', '#2A3241');
          safeSetPaintProperty(layer, 'fill-opacity', 0.8);
        });
      }
          ...
          'features': [
            {
              ...
              'geometry': {
                'type': 'Point',
                'coordinates': [77.808946, 13.126281]
              }
            }
          ]
        }
    ...
    >
      ...
      <div className="mt-10 relative">
        ...
        >
          ...
          >
            Currently living in the dynamic and bustling city of Bangalore, where cutting-edge innovation meets rich cultural heritage.
          </motion.p>
        </motion.div>
      </div>
    </motion.div>
