...
export default function NowPlaying() {
  ...
  const [loadingParticleConfigs] = useState(() =>
    Array.from({ length: 8 }, () => ({
      width: Math.random() * 2 + 1,
      height: Math.random() * 2 + 1,
  ...
  // Fetch and poll for Spotify data - FIXED: removed track from dependencies to prevent infinite loop
  useEffect(() => {
      ...
      try {
        ...
        if (data && data.item) {
          setTrack((currentTrack) => {
            ...
            if (!currentTrack || data.item.id !== currentTrack.item.id) {
              if (currentTrack) setPrevTrack(currentTrack);
              ...
              // Start new progress tracking if playing (optimized to 100ms for better performance)
              if (data.is_playing && data.item.duration_ms) {
                progressInterval.current = setInterval(() => {
                  setProgress(prev => {
                    const newProgress = prev + (100 / data.item.duration_ms);
                    return newProgress > 1 ? 1 : newProgress;
                  });
                }, 100); // Reduced from 10ms to 100ms for 10x better performance
              }

              return data;
            }

            // Same track, check if play/pause status changed
            if (currentTrack.is_playing !== data.is_playing) {
              // Handle play/pause (optimized progress tracking)
              if (data.is_playing && data.item.duration_ms) {
                setProgress(data.progress_ms / data.item.duration_ms);
      ...
      <div className="flex flex-col items-center relative">
        ...
        >
          <div className="flex items-center justify-center gap-1 relative">
            ...
            {[...Array(7)].map((_, i) => {
              const baseHeight = [5, 10, 15, 20, 15, 10, 5][i] ?? 10;
              const delay = i * 0.1;
            ...
            {isMounted && [...Array(8)].map((_, i) => {
              const config = loadingParticleConfigs[i];
  ...
  return (
    <div className="flex flex-col items-center relative">
      ...
      {/* Animated particles */}
      {isMounted && track && track.is_playing && [...Array(12)].map((_, i) => {
