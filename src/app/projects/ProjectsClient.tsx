...
export default function Projects() {
  ...
  const bgParticles = useClientParticles(30, (i) => ({
    ...
    top: Math.random() * 100,
    r: 52 + Math.random() * 30,
    g: 211 + Math.random() * 30,
    b: 153 + Math.random() * 30,
    alpha: 0.2 + Math.random() * 0.3,
    blur: Math.random() * 5 + 2,
    shadowAlpha: 0.3 + Math.random() * 0.4,
    yMovement: -Math.random() * 150 - 50,
        ...
        {bgParticles.length > 0 && bgParticles.map((particle, i) => (
          <motion.div
            ...
            style={{
              ...
              left: `${particle.left}%`,
              top: `${particle.top}%`,
              background: `rgba(${particle.r}, ${particle.g}, ${particle.b}, ${particle.alpha})`,
              filter: `blur(${particle.blur}px)`,
              boxShadow: `0 0 ${particle.blur}px rgba(52, 211, 153, ${particle.shadowAlpha})`,
            }}
            animate={{
              opacity: [0, 0.8, 0],
        ...
        >
          ...
          {categories.map((category, index) => (
            ...
            >
              ...
              {category === activeCategory && btnParticles.length > 0 && (
                <>
                  {btnParticles.map((particle, i) => (
                    <motion.div
                      key={`btn-particle-${i}`}
        ...
        >
          ...
          <div className="text-center pt-12 border-t border-zinc-800/30">
            ...
            <div className="relative w-full max-w-xs mx-auto mb-8">
              ...
              {lineParticles.length > 0 && lineParticles.map((particle, i) => (
                <motion.div
                  key={`line-particle-${i}`}
                  className="absolute top-0 h-[2px] w-[2px] rounded-full bg-green-300"
                  initial={{ left: `${particle.left}%`, opacity: 0 }}
                  animate={{
                    opacity: [0, 1, 0],
                    y: [-1, 1, -1],
                    boxShadow: [
            ...
            >
              ...
              >
                ...
                {signParticles.length > 0 && signParticles.map((particle, i) => (
                  <motion.div
                    key={`sign-particle-${i}`}
