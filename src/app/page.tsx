...
const reportWebVitals = (metric: MetricType) => {
  // Log to console for monitoring (can be sent to analytics later)
...
const LocationSection = dynamic(() => import("~/components/LocationSection"), {
  ssr: false,
  loading: () => <div className="h-96 bg-zinc-900/30 rounded-lg animate-pulse" /> // Skeleton loader
});
...
export default function HomePage() {
  ...
  // Calculate if animations should run (only when page is visible and user doesn't prefer reduced motion)
  const shouldAnimateParticles = useMemo(() =>
    isPageVisible && !prefersReducedMotion,
    [isPageVisible, prefersReducedMotion]
  );

  // Generate stable random values for particles on client side only (optimized counts)
  // Wrapped in useMemo to prevent recalculation on re-renders
  const avatarParticleConfigs = useMemo(() =>
    Array.from({ length: 12 }, () => ({ // Reduced from 20 to 12 for better performance
  ...
  const clickParticleConfigs = useMemo(() =>
    Array.from({ length: 5 }, () => ({ // Reduced from 8 to 5 for better performance
  ...
  const pulseParticleConfigs = useMemo(() =>
    Array.from({ length: 3 }, (_, i) => ({
      backgroundColor: `hsl(${142 + Math.random() * 30}, ${80 + Math.random() * 20}%, ${70 + Math.random() * 20}%)`,
      scale: Math.random() + 0.5,
      x: (Math.random() - 0.5) * 30 * (i + 1),
      y: (Math.random() - 0.5) * 30 * (i + 1),
      duration: 0.8 + Math.random() * 0.5,
      repeatDelay: Math.random() * 0.5,
    })), []
  );

  const aboutParticleConfigs = useMemo(() =>
    Array.from({ length: 5 }, () => ({ // Reduced from 8 to 5 for better performance
      top: `${Math.random() * 100}%`,
  ...
  const educationParticleConfigs = useMemo(() =>
    Array.from({ length: 5 }, () => ({ // Reduced from 8 to 5 for better performance
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
  ...
  const experienceParticleConfigs = useMemo(() =>
    Array.from({ length: 5 }, () => ({ // Reduced from 8 to 5 for better performance
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
  ...
  const skillsParticleConfigs = useMemo(() =>
    Array.from({ length: 4 }, () => ({ // Reduced from 6 to 4 for better performance
      backgroundColor: `hsl(${142 + Math.random() * 30}, ${80 + Math.random() * 20}%, ${70 + Math.random() * 20}%)`,
      scale: Math.random() + 0.5,
  ...
  const hobbiesParticleConfigs = useMemo(() =>
    Array.from({ length: 5 }, () => ({ // Reduced from 8 to 5 for better performance
      top: `${Math.random() * 100}%`,
      left: `${Math.random() * 100}%`,
      y: (Math.random() - 0.5) * 10,
  ...
  // Memoize animation variants to prevent recreation on re-renders
              ...
              <div className="relative overflow-hidden rounded-full flex items-center justify-center border-2 border-green-300/30 z-10">
                ...
                >
                  ...
                  >
                    ...
                    {/* Cosmic particles explosion - paused when tab hidden or reduced motion enabled */}
                    <ClientOnly>
                      {shouldAnimateParticles && [...Array(20)].map((_, i) => {
                        const config = avatarParticleConfigs[i];
            ...
            <div className="text-center sm:text-left mt-2 sm:mt-0">
                  ...
                  <span className="relative inline-block">
                    {'Shreyas'.split('').map((letter, i) => (
                      ...
                      >
                        ...
                        >
                          <ClientOnly>
                            {shouldAnimateParticles && [...Array(8)].map((_, j) => {
              ...
              >
                ...
                >
                  ...
                  >
                    <ClientOnly>
                      {shouldAnimateParticles && [...Array(5)].map((_, i) => {
